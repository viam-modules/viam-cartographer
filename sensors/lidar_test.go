...
func TestNewLidar(t *testing.T) {
	...
	t.Run("No lidar provided", func(t *testing.T) {
		...
		_, err := s.NewLidar(context.Background(), deps, lidarName, testDataFrequencyHz, logger)
	...
	t.Run("Failed lidar creation with non-existing sensor", func(t *testing.T) {
		lidarName := "gibberish"
		deps := s.SetupDeps(lidarName, "")
		actualLidar, err := s.NewLidar(context.Background(), deps, lidarName, testDataFrequencyHz, logger)
		test.That(t, err, test.ShouldBeError,
			errors.New("error getting lidar camera "+
	...
	t.Run("Successful lidar creation", func(t *testing.T) {
		...
		deps := s.SetupDeps(lidarName, "")
		actualLidar, err := s.NewLidar(ctx, deps, lidarName, testDataFrequencyHz, logger)
...
func TestValidateGetLidarData(t *testing.T) {
	...
	lidarName := "good_lidar"
	goodLidar, err := s.NewLidar(ctx, s.SetupDeps(lidarName, ""), lidarName, testDataFrequencyHz, logger)
	test.That(t, err, test.ShouldBeNil)

	lidarName = "lidar_with_invalid_properties"
	_, err = s.NewLidar(ctx, s.SetupDeps(lidarName, ""), lidarName, testDataFrequencyHz, logger)
	...
	t.Run("returns nil if a lidar reading succeeds within the timeout", func(t *testing.T) {
		...
		warmingUpLidar, err := s.NewLidar(ctx, s.SetupDeps(lidarName, ""), lidarName, testDataFrequencyHz, logger)
		test.That(t, err, test.ShouldBeNil)
...
func TestTimedLidarSensorReading(t *testing.T) {
	...
	lidarWithErroringFunctions, err := s.NewLidar(ctx, s.SetupDeps(lidarName, ""), lidarName, testDataFrequencyHz, logger)
	test.That(t, err, test.ShouldBeNil)

	lidarName = "invalid_replay_lidar"
	invalidReplayLidar, err := s.NewLidar(ctx, s.SetupDeps(lidarName, ""), lidarName, testDataFrequencyHz, logger)
	test.That(t, err, test.ShouldBeNil)
	...
	goodReplayLidar, err := s.NewLidar(ctx, s.SetupDeps(lidarName, ""), lidarName, testDataFrequencyHz, logger)
	test.That(t, err, test.ShouldBeNil)
